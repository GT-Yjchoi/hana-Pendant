...
class TopBar(QFrame):
    ...
    def __init__(self, plc_client=None):
        ...
        # 2. 통신 상태 (COMM)
        self.lbl_comm = QLabel("COMM: --")
        self.lbl_comm.setStyleSheet("color: #95A5A6; font-size: 15px; font-weight: bold;")
        layout.addWidget(self.lbl_comm)

        # 3. 운전 모드 (MODE)
        self.lbl_mode = QLabel("MODE: STOP")
        self.lbl_mode.setStyleSheet("color: #95A5A6; font-size: 16px; font-weight: bold;")
        layout.addWidget(self.lbl_mode)

        # 4. 알람 상태 (ALARM) - 클릭 시 오버레이 재표시
        self.lbl_alarm = QLabel("ALARM: NONE")
        self.lbl_alarm.setStyleSheet("color: #95A5A6; font-size: 15px; font-weight: bold;")
        ...
        # 5. 금형 데이터
        self.lbl_mold = QLabel("MOLD: Default")
        self.lbl_mold.setStyleSheet("color: #FFFFFF; font-size: 18px; font-weight: bold;") 
        ...
        self.lbl_time.setStyleSheet("color: #E5E7EB; font-size: 16px; font-weight: 600;")
    ...
    def set_comm_status(self, connected):
        if connected:
            self.lbl_comm.setText("COMM: OK")
            self.lbl_comm.setStyleSheet("color: #00E5FF; font-size: 15px; font-weight: bold;")
        else:
            self.lbl_comm.setText("COMM: ERR")
            self.lbl_comm.setStyleSheet("color: #FF4646; font-size: 15px; font-weight: bold;")

    # ★ 에러 해결: 금형 데이터 설정 함수
    def set_mold_data(self, name):
        self.lbl_mold.setText(f"MOLD: {name}")
    ...
    def _on_monitor_data(self, data):
        ...
        if axis_alarms:
            if 9 in axis_alarms:
                self.lbl_alarm.setText("[!] E-STOP")
            else:
                self.lbl_alarm.setText(f"[!] ALARM ({len(axis_alarms)}축)")
            self.lbl_alarm.setStyleSheet(
                "color: #FF4646; font-size: 15px; font-weight: 900; text-decoration: underline;"
            )
        else:
            self.lbl_alarm.setText("ALARM: NONE")
            self.lbl_alarm.setStyleSheet("color: #95A5A6; font-size: 15px; font-weight: bold;")